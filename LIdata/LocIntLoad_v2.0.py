...
tagname = input('input hospital name: ')

db = sqlite3.connect(tagname + '_GEOCODE.sqlite')  # Make sure to update to new SQL database
cur = db.cursor()
...
fh = open(tagname + "DB.csv")  # Make sure to update to new CSV or TXT file
count = 0
for line in fh:
    if count == 500 :  # Retrieves 500 items at a time
...
print("Run LocIntDump.py to read the data from the database and write it into a CSV file.")
